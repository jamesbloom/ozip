...
void print_help() 
{
	...
	if (!g_bequiet)
	{
		...
		fprintf(stderr, "-F --fast               low latency streaming compression\n");
		fprintf(stderr, "-b --best               best compression. Long compress times.\n");
		...
		fprintf(stderr, "-(1-9)	                compression level 1-9\n");
...
#ifdef PLATFORM_UNIX
		fprintf(stderr, "--timeout=[1000]		time in ms to wait on inactve stdin during compression\n");
#endif
...
void print_oodle_help()
{
	...
	if (!g_bequiet)
	{
		...
		fprintf(stderr, "      -o?   --oodlelist                           list Oodle compression enum values\n");
		fprintf(stderr, "      -oc#  --compressor=# { 8,9,11,13 }          compressor choice\n");
		fprintf(stderr, "      -ol#  --level=# [0, 8]                      compression level\n");
		fprintf(stderr, "      -os#  --spacespeedtradeoff=# [64, 1024]     smaller -> more compression, slower decodes\n");
		...
		fprintf(stderr, "      -oo#  --offsetlimit=# {0, [256,...]}        max context dictionary size in kb\n");
...
void set_minmatchlen(char * valuepoint)
{
...
void set_seekchunklen(char * valuepoint)
{
	int parsedval = atoi(valuepoint);
...
void set_offsetlimit(char * valuepoint)
{
	int parsedval = atoi(valuepoint);
	if (parsedval >= 0)
	{
...
void set_localdict(char * valuepoint)
{
	int parsedval = atoi(valuepoint);
...
void set_matchtablesize(char * valuepoint)
{
	int parsedval = atoi(valuepoint);
...
void handle_oodle_opt( char * opt_in_arg)
{
	char * valuepoint = opt_in_arg + 1;
	if (*valuepoint == '=')
	{
		valuepoint += 1;
	}
...
char * compare_end(char  * str1, const char * str2)
{
...
void set_compressor(char * valuepoint)
{
...
{
	const char  shortoptions[] = "HhdzKkmftcqvLVs123456789fb=";
	...
	const char * longoptions[] = { "help","decompress","force","keep","quiet","test","verbose","best" };
	const int longoptcount = 8;
	for (int optind = 1; optind < argc; optind++)
	{
		if (argv[optind][0] == '-')
		{
		...
		else
		{
			...
			if (!g_testfile)
			{
				if (success && g_verify)
				{
					...
					bool decompverified = decompress(originalinfile);
					if (originalinfile) fclose(originalinfile);
					if (!decompverified)
					{
...
int main(int argc, char * argv[]) 
{
	...
	if (!g_printedhelp)
	{
		g_scratchmemsize = max(g_scratchmemsize, OodleLZDecoder_MemorySizeNeeded(OodleLZ_Compressor_Invalid, -1));
